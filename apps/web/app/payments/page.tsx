    ...
    try {
      ...
      if (paymentType === 'cross-border' && limits) {
        ...
        if (amountSol > remaining) {
          throw new Error(
            `Cross-border limit exceeded. Remaining: ${remaining.toFixed(2)} SOL`
          );
        }
      }
...
function TransactionItem({ transaction }: { transaction: Transaction }) {
  ...
  const date = transaction.timestamp ? new Date(transaction.timestamp * 1000).toLocaleDateString() : 'N/A';
  const time = transaction.timestamp ? new Date(transaction.timestamp * 1000).toLocaleTimeString() : '';

  return (
    <div className={`${config.bgColor} rounded-lg p-4 border border-stone-200 dark:border-stone-800`}>
