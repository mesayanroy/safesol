# 🔐 Protocol Guarantees: ZK Private Payments on Solana
...
Circuit Constraint: balance ≥ amount
...
// nullifier_A ≠ nullifier_B → Both txs pass double-spend check
```
...
```
TX submitted → Broadcast to validators → Validators execute TX →
Block produced → Finality (6+ blocks) → State committed
```
...
- PDAs are derived deterministically: `[seeds] → address`
...
```
PDA Derivation: sha256(["state"], program_id) → state_address
...
### Tier 3: Abstracted but Real (Demo Architecture) 🧪
...
| Commitment hash       | 5ms        | —          | —      |
| ZK proof generation   | 500–2000ms | 256 bytes  | —      |
| On-chain verification | ~10ms      | —          | 10K CU |
| Merkle root update    | ~100ms     | —          | 5K CU  |
...
- ✅ Identities hidden via nullifiers
- ✅ Selective disclosure for auditors

### Security

- ✅ Double-spend prevention (nullifier)
- ✅ Balance enforcement (ZK constraint)
- ✅ State integrity (on-chain PDA)

### Auditability

- ✅ All transactions on-chain
- ✅ Privacy receipts for compliance
- ✅ Merkle root verifiable on explorer
...
| ------------------------- | ---------- | --------------------- |
| Real Groth16 verification | ⏳ Ready   | Removes mock verifier |
| Mainnet deployment        | ⏳ Ready   | Real economic value   |
| Multi-signature approvals | 📋 Planned | Institutional use     |
| Threshold encryption      | 📋 Planned | Selective disclosure  |
| Hardware wallet support   | 📋 Planned | Enhanced security     |
