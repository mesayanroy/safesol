...
use safesol_compressed_pda::{CounterCompressedAccount};
use solana_sdk::{
...
async fn test_counter_program() {
    let config = ProgramTestConfig::new(true, Some(vec![("safesol_compressed_pda", safesol_compressed_pda::ID)]));
...
async fn create_counter(
    ...
    let config = SystemAccountMetaConfig::new(safesol_compressed_pda::ID);
    ...
    let instruction = Instruction {
        program_id: safesol_compressed_pda::ID,
        accounts: [
        ...
        data: {
            ...
            safesol_compressed_pda::instruction::Create {
...
async fn increment_counter(
    ...
    let instruction = Instruction {
        ...
        data: {
            use anchor_lang::InstructionData;
            safesol_compressed_pda::instruction::Increment {
                proof: rpc_result.proof,
...
async fn delete_counter(
    ...
    let instruction = Instruction {
        ...
        data: {
            use anchor_lang::InstructionData;
            safesol_compressed_pda::instruction::Delete {
                proof: rpc_result.proof,
                counter_value: counter_account.counter,
                account_meta: CompressedAccountMeta {
                    tree_info: packed_tree_accounts.packed_tree_infos[0],
